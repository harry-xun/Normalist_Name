...
class NamesAPI:
    ...
    def get_random_starting_name(self):
        self.cursor.execute("SELECT _name FROM names ORDER BY RANDOM() LIMIT 1")
        random_name = self.cursor.fetchone()
    ...
    def get_close_name(self, name, year):
        ...
        if result:
            ...
            while True:
                close_rank = name_rank + random.randint(-100, 100)
    ...
    def get_popularity_and_sex(self, name, lower_bound_year, upper_bound_year):
        """
        Retrieves the popularity and gender of the given name between the specified years.
        """
        if not self.validate_input_year(lower_bound_year, 0, 1) or not self.validate_input_year(upper_bound_year, 0, 1) or not self.validate_input_name(name):
            return []
    
        query = """SELECT _year, amount, sex FROM names WHERE _name = %s AND _year BETWEEN %s AND %s ORDER BY _year;"""
        self.cursor.execute(query, (name, lower_bound_year, upper_bound_year))
    ...
    def generate_name_from(self, lower_bound_year, upper_bound_year, lower_bound_popularity, upper_bound_popularity):
        ...
        query = """SELECT _name, amount FROM (SELECT DISTINCT ON (_name) _name, amount FROM names WHERE _year BETWEEN %s AND %s ORDER BY _name, amount DESC) AS subquery ORDER BY amount DESC OFFSET %s LIMIT %s;"""
...
if __name__ == "__main__":
 ...
 #   print(nameFinder.get_popularity_of("Alison",1941))
 #   print(nameFinder.get_popularity_of("Geoffrey",1941))
...
#    nameFinder.play_game()
    print(nameFinder.get_popularity_and_sex('Harry',1890,2022))
